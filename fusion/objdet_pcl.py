...
def show_pcl(pcl):
    ...
    vis_calls = o3d.visualization.VisualizerWithKeyCallback()
    vis_calls.create_window(window_name='Open3D', width=1920, height=1080, left=50, top=50, visible=True)
    ...
    vis_calls.register_key_action_callback(262, right_arrow)

    #screen = pygame.display.set_mode((320, 200))
    running = False
    def right_arrow(vis_calls):
...
def show_range_image(frame, lidar_name):
    ...
    # step5: Crop range image to +/- 90 deg. left and right of the forward-facing x-axis, focus on +/- 90° around the image center
    deg90 = int(img_range.shape[1] / 4)
    ri_center = int(img_range.shape[1]/2)
    img_range = img_range[:,ri_center-deg90:ri_center+deg90]
    
    # step 6 : map the intensity channel onto an 8-bit scale and normalize with the difference between the 1- and 99-percentile to mitigate the influence of outliers
    ri_intensity = ri[:,:,1]
    ri_intensity = 255 * np.clip(ri_intensity, np.percentile(arr, 1), np.percentile(arr, 99)) / np.percentile(arr, 99)
    img_intensity = ri_intensity.astype(np.uint8)

    # step 7 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    # but make sure that no negative bev-coordinates occur
    ## transform all metrix y-coordinates as well but center the foward-facing x-axis
